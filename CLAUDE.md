...
- 2026-09-09 (ícone de verdade): usuário mandou `icones.zip` com 4 tamanhos (80/160/240/320px) de um design feito no Figma — sparkle branco sobre gradiente quente (creme → pêssego/laranja), bem mais bonito que qualquer coisa que eu ia desenhar.
  - Fonte salva em `assets/icon-source.png` (o maior, 320x320).
  - Testei upscale pra 1024 antes de assumir que dava: como o design é todo gradiente suave sem linha dura, ampliar 3.2x não gerou serrilhado visível. Não precisou pedir arquivo maior ao usuário.
  - **`app/make-icon.sh`**: gera as 10 resoluções que o `iconutil` exige (16 a 1024, incluindo @2x) a partir da fonte única, empacota em `.icns` via `iconutil -c icns`. Chamado automaticamente pelo `package.sh`; `CFBundleIconFile` adicionado ao `Info.plist`.
  - **Confirmado visualmente**: montei o `.dmg` de novo e vi o ícone de verdade (não mais o genérico cinza) tanto na janela do DMG quanto na pasta Aplicativos. `sips -g all` no `.icns` também confirmou 1024x1024, RGBA, perfil sRGB válido.
  - Sem crash depois de trocar o `Info.plist`.
