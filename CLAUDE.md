...
- 2026-09-09 (toggle voltou atrás): usuário testou o `Toggle` nativo ao vivo e achou ruim ("ficou muito ruim com o toggle"). Pediu um indicador redondo no lugar: bolinha azul preenchida quando ligado, só contorno quando desligado — nem o switch nativo, nem o check antigo. Implementado como `DotToggle` (view customizada, `Circle` preenchido ou com stroke). Também: subtítulo do "Force max volume" encurtado pra uma linha ("Briefly overrides your Mac's volume for every alert — also affects music or calls playing at that moment."), espaçamento entre título e subtítulo aumentado (4→6, +2 de padding no subtítulo), e o título desse item foi pra 15pt (os outros continuam 13pt). Recompilado, reempacotado, sem crash. Commitado e pronto pra push.
